...
| C36 | **hit** | `pathcheck.py` over **625 tracked files: 0 violations, positive control firing, negative control quiet**, run with the rule-0 log present; third parties' paths published as findings; and a scratch note carrying an absolute path was deleted and regenerated relative rather than committed |
