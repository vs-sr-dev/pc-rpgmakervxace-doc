...
# pc-rpgmakervxace-doc is the first object in this collection with a file name
# outside Latin-1, and this tool's whole job is to print file names. Without
# this line it dies of UnicodeEncodeError partway down a 2,026-row table, on a
# Japanese PDF name in a downloadable-content pack -- having already printed
# 1,100 rows, so the failure looks like a truncated file rather than a crash.
# The box's convention (a tool that prints recovered text sets its own output
# encoding) had been applied to recovered TEXT and never to NAMES.
nameguard.guard()
