...
def redact(data):
    ...
    Both encodings are swept and the counts are added, so `--expect` counts
...
def counts(data):
    """(eight-bit, UTF-16) separately, for a report that has to say which pass
    found what."""
...
def cmd_run(args):
    ...
    if args.check:
        ...
        print("check     : %d address-shaped strings remain in the file "
              "(%d eight-bit, %d UTF-16)" % (n8 + n16, n8, n16))
        return 1 if (n8 + n16) else 0
    if args.in_place:
...
def cmd_selftest(args):
    ...
    n8, n16 = counts(b"a@b.invalid " + wide)
    checks.append(("the two passes are counted separately",
                   (n8, n16) == (1, 1), str((n8, n16))))
