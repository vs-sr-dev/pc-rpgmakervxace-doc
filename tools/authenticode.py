...
def main():
    ...
    # The data directories do not live at a constant offset. In a PE32
    # optional header they begin 96 bytes in; in a PE32+ one, 112, because
    # BaseOfData is gone and four size fields widen from 4 bytes to 8. This
    # line read 96 unconditionally for forty-four copies of this tool, and it
    # was never wrong because it had never been given a 64-bit binary.
    #
    # On pc-themurderofsonicthehedgehog-doc it was given six. On
    # UnityPlayer.dll it read the 96-byte layout out of a 112-byte header,
    # landed on SizeOfHeapCommit, announced a certificate table at 29,769,728
    # in a file of 28,885,424 bytes -- 884,304 past the end -- and crashed
    # reading there. `_work/certdir.py` had already checked all 137 PE files
    # against their own sizes and found ZERO that declare a table past the
    # end, so the overshoot was the tool's and not the file's.
    magic = struct.unpack_from("<H", d, e + 24)[0]
    if magic == 0x20b:
        ddoff, kind, ndir_off = e + 24 + 112, "PE32+", e + 24 + 108
    elif magic == 0x10b:
        ddoff, kind, ndir_off = e + 24 + 96, "PE32", e + 24 + 92
    else:
        raise SystemExit("authenticode: optional header magic 0x%04X is "
                         "neither PE32 (0x10b) nor PE32+ (0x20b)" % magic)
    ndirs = struct.unpack_from("<I", d, ndir_off)[0]
    if ndirs < 5:
        print("file            : %s" % a.path)
        print("file size       : %d" % len(d))
        print("%s, %d data directories -- there is no SECURITY entry"
              % (kind, ndirs))
        return
    off, size = struct.unpack_from("<II", d, ddoff + 4 * 8)
    # And the bound the crash proved was needed. A declared table that does
    # not fit inside the file is reported and refused rather than read.
    if off and (off >= len(d) or off + size > len(d)):
        print("file            : %s" % a.path)
        print("file size       : %d" % len(d))
        print("cert table      : file offset %d (0x%X), %d bytes" % (off, off, size))
        raise SystemExit("authenticode: that table ends %d bytes past the end "
                         "of the file; refusing to read it"
                         % (off + size - len(d)))

    print("file            : %s" % a.path)
    print("file size       : %d" % len(d))
    print("optional header : %s, %d data directories" % (kind, ndirs))
